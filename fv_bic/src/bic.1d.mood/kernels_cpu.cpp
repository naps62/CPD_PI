...
void cpu_compute_u(CFVMesh2D &mesh, CFVRecons2D &recons, CFVArray<double> &polution, CFVArray<double> &p, double CFL) {
	...
	for(int edge = 0; edge < mesh.num_edges; ++edge) {
		...
		if (recons.degree[left] == 1) recons.u_ij[edge] = polution[left]  + p[left] * mesh.cell_areas[left] * 0.5 * (1 - CFL);
		else                          recons.u_ij[edge] = polution[left];

		if (recons.degree[right] == 1) recons.u_ji[edge] = polution[right] - p[right] * mesh.cell_areas[right] * 0.5 * (1 - CFL);
		else                           recons.u_ij[edge] = polution[right];
	}
}
...
bool cpu_mood_detector(CFVMesh2D &mesh, CFVRecons2D &recons, CFVArray<double> &polution, CFVArray<double> &candidate) {
	...
	for(unsigned int cell = 0; cell < mesh.num_cells; ++cell) {
		...
		double p_left  = polution[left];
		double p_cell  = polution[cell];
		double p_right = polution[right];
		double p_cand  = candidate[cell];

		double p_min = _min(p_left, _min(p_cell, p_right));
		double p_max = _max(p_left, _max(p_cell, p_right));

		if (!(p_min <= p_cand && p_cand <= p_max)) {
