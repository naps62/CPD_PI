...
void alloc_partitions(FVMesh2D_SOA &mesh, FVArray<double> &v, FVArray<double> &polu, vector<PartitionData> &partitions, FVMesh2D_SOA_Lite &result, int id) {
	...
	for(set<unsigned int>::iterator cell_it = part_data.cells.begin(); cell_it != part_data.cells.end(); ++cell_it) {
		...
		result.polution			[cells_current] = polu[cell];
	...
	for(unsigned int edge = 0; edge < result.num_edges; ++edge) {
		...
		// if cell exists in left partition
			...
			result.edge_part_index[edge] = result.left_cell_count++;
		}

		// by exclusion, it can only exist in the right partition
		else {
			result.edge_part[edge] = 1;
			result.edge_part_index[edge] = result.right_cell_count++;
		}
	}
	result.left_cells_send  = new CFVArray<double>(result.left_cell_count);
	result.right_cells_send = new CFVArray<double>(result.right_cell_count);
	result.left_cells_recv  = new CFVArray<double>(result.left_cell_count);
	result.right_cells_recv = new CFVArray<double>(result.right_cell_count);
   ...
   // fix edges
	...
	result.left_index_to_edge  = new CFVArray<unsigned int>(result.left_cell_count);
	result.right_index_to_edge = new CFVArray<unsigned int>(result.right_cell_count);
	for(unsigned int e = 0; e < result.num_edges; ++e) {
