...
\todonaps{(FIXED. o sentido deste paragrafo era explicar que a remoçao da redução, que já era feita num kernel á parte, acaba por optimizar indirectamente a compute\_flux, que agora já não precisa de calcular as velocidades) Honestamente que não percebi a primeira frase, nem percebi completamente o sentido deste parágrafo. Acho que precisa de ser reescrito.

Quanto ao que foi efectivamente escrito, ``outside the main loop'' pode ser substituido por preprocessing stage (não literalmente ofc, mas tu percebes). E cuidado com o didn't na primeira frase.}
