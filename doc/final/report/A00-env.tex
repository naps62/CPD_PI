...
Nodes in SeARCH Group Hex have two hex-core processors (with \intel Hyper-threading technology) and 12 to 48 GB of RAM.Further detail regarding the hardware of these nodes can be found in \cref{tab:grouphex}.

These nodes were used for every test performed in this document.
Sequential, shared memory and GPU tests relied used only one node, where tests for distributed memory used two distinct nodes. Inside this group, tests were only limited to the subset with the highest processor clock frequency, not making any distinction between the nodes.
